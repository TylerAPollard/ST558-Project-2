...
hour.data$temp.F <- hour.data$temp.unnormal*(9/5) + 32 # Convert to Farenheit
...
hour.data$atemp.F <- hour.data$atemp.unnormal*(9/5) + 32 # Convert to farenheit
...
fit.aic2 <- step((lm(cnt ~ .^2 + I(temp^2) + I(atemp^2) + I(hum^2) + I(windspeed^2), data = hour.training.data2, verbose = FALSE)), direction = "both")
...
```{r models - linear regression 1}
set.seed(7)
# fit linear regression model
fit.lm1 <- train(fit.aic$terms,
                  data = hour.training.data2,
                  method = "lm",
                  trControl = trainControl(method = "cv", number = 10))
fit.lm1

# Predict test count on test data
predict.lm1 <- predict(fit.lm1, newdata = hour.test.data)

# Determine performance of linear model fit on test data
performance.lm1 <- postResample(predict.lm1, obs = hour.test.data$cnt)
performance.lm1
```
...
#                            trControl = trainControl(method = "cv", number = 10),
#                            verbose = FALSE,
#                            tuneGrid = data.frame(mtry = seq(10, 30, by = 2)))
# fit.random.forest

# Predict count on test data using random forest fit midel
predict.rf <- predict(fit.random.forest.trial, newdata = hour.test.data)

# Examine performance of random forest model on the test data
performance.rf <- postResample(predict.rf, obs = hour.test.data$cnt)
performance.rf
```
...
compare.rmse <- data.frame(predict.mlr0, 
                           ...
                           performance.rf,
                           predict.boosted)
colnames(compare.rmse) <- c("predict.mlr0", "predict.mlr1", "predict.mlr2", "predict.mlr3", "perofrmance.lm1", "performance.rf", "predict.boosted")
compare.rmse
```
...
colnames(compare.rmse)[min.test]
```
